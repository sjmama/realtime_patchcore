...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        image = to_pil_image(image_path) if isinstance(image_path, torch.Tensor) else PIL.Image.open(image_path).convert("RGB")
        ...
        if masks_provided:
            if mask_path is not None:
                mask =  to_pil_image(mask_path) if isinstance(mask_path, torch.Tensor) else PIL.Image.open(mask_path).convert("RGB")
        ...
        if num==0:
            savename = image_path.split("/")###############imagepath 수정으로 인한 수정
        ...
        plt.title(f'{num}-{anomaly_score}', loc='right', pad=20)
